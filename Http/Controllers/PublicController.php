...
class PublicController extends BasePublicController
{
    ...
    public function uri($slug)
    {
        ...
        // Si la pagina no existe, pagina personalizada 404
        if (null === $page || $page->status === $this->disabledPage) {
            //Si la web esta desactivada
            if(Setting::get('core::active')==0){
                if($this->auth->hasAccess('page.pages.index')){
                    $page = $this->findPageForSlug('error-404');
